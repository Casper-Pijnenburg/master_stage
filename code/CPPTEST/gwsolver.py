...
class GWSolver():
    def __init__(self,
        ...
        self.sigma_w = int(self.fock_flag) * self.fock(self.V, self.rho_w) + int(self.hartree_flag) * self.hartree(self.V, self.rho_w) + self.selfEnergy(self.g0_t, self.W_w)
    ...
    def screenedPotential(self, V, P_w):
        ...
        if not self.self_interactions:
            for i, w in enumerate(V_t.mesh.values()):
                for j in range(self.target_shape[0]):
                    V_t.data[i, j, j] = 0

        I = P_w['up'].copy()
        I.data[:] = np.eye(self.target_shape[0])
        ...
        W['up'] = (A_inv + A_inv * B * S * C * A_inv) * V_t - A_inv * B * S * V['up'];
        W['dn'] = -S * C * A_inv * V['up'] + S * V_t;
    ...
    def selfEnergy(self, g0_t, W_w):
        ...
        W_dynmatic = W_w - V
        W_dynamic_t_dlr = make_gf_dlr(W_dynmatic)
    ...
    def fock(self, V, rho):
        ...
        fock = rho.copy()
        for block in self.blocks:
            fock[block].data[:, :, :] = -V_t[block].data[:-1, :, :] * rho[block].data[:, :, :]
