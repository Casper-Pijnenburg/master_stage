...
class GWSolver():
    def __init__(self,
                 ...
                 N_fix = None, fix_g0 = True, fix_g = False, print_occupation = False, full_mesh = False):
        

        self.self_interactions, self.hartree_flag, self.fock_flag = self_interactions, hartree_flag, fock_flag
        self.full_mesh = full_mesh
        self.fix_g0, self.fix_g = fix_g0, fix_g
        self.print_occupation = print_occupation
        ...
        if self.fix_g0 and self.N_fix is not None :
            self.g0_w = self.fix_particle_number(g0_w, self.N_fix)
        else:
            self.g0_w = g0_w

        if self.print_occupation:
            print(f'occupation g0_w: {self.N(self.g0_w)}')
        ...
        if self.fix_g and self.N_fix is not None:
            self.g_w = self.fix_particle_number(self.oneShotG(self.g0_w, self.sigma_w), self.N_fix)
        else:
            self.g_w = self.oneShotG(self.g0_w, self.sigma_w)

        if self.print_occupation:
            print(f'occupation g_w: {self.N(self.g_w)}')

        if self.full_mesh:
            n_iw = 1024

            P_dlr = make_gf_dlr(self.P_w)
            self.P_w = make_gf_imfreq(P_dlr, n_iw)

            W_dlr = make_gf_dlr(self.W_w)
            self.W_w = make_gf_imfreq(W_dlr, n_iw)

            sigma_dlr = make_gf_dlr(self.sigma_w)
            self.sigma_w = make_gf_imfreq(sigma_dlr, n_iw)
